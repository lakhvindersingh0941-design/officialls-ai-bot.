...
with st.sidebar:
    ...
    # --- ASSET SELECTION (BTC or ETH) ---
    asset = st.selectbox("Select Asset to Trade", ["BTCUSDTPERP", "ETHUSDTPERP"])
    ...
    st.divider()
    # AUTO TRADING SWITCH
    auto_trade = st.toggle(f"🚀 AUTO AI {asset[:3]} TRADING ON", value=False)
...
if conn_status == "SUCCESS":
    st.success(f"✅ Connected to Delta India | Trading: {asset}")
...
with col_sig:
    ...
    st.divider()
    st.subheader("📊 Config")
    st.write(f"Fees: 0.1% | Asset: {asset[:3]}")
    st.write("SL: 0.8% | TP: 1.5%")
    st.divider()
    p_sigs = st.empty()

with col_main:
    # TradingView Chart Dynamic
    ...
    try:
        ...
        if conn_status == "SUCCESS":
            ...
            try:
                ...
                if pos:
                    ...
                    p_positions.info(f"No Active Real {asset[:3]} Positions")
            except: pass
        else:
        ...
        if auto_trade and st.session_state.last_p != 0 and abs(price - st.session_state.last_p) > (0.5 if "ETH" in asset else 1.5):
            ...
            st.session_state.history.insert(0, {
                "Time": ist, "Asset": asset[:3], "Side": "LONG" if price > st.session_state.last_p else "SHORT",
        ...
        p_sigs.code(f"🔴 SELL {asset[:3]}: {price + (1 if 'ETH' in asset else 3)}\n🟢 BUY {asset[:3]}:  {price - (0.5 if 'ETH' in asset else 2)}")
