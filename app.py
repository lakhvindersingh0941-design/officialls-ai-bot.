...
with st.sidebar:
    ...
    mode = st.radio("Account Mode", ["Demo ($10 Simulation)", "Real Delta Account"])
    
    api_k = st.text_input("Delta API Key", type="password") if mode == "Real Delta Account" else ""
    api_s = st.text_input("Delta API Secret", type="password") if mode == "Real Delta Account" else ""
...
# 3. Connection Logic (Fixed for 'Invalid' Error)
        ...
        try:
            ex = ccxt.delta({
                ...
                'options': {
                    ...
                    'recvWindow': 30000 # Connection delay tolerance
                }
            })
            ex.fetch_balance()
            return ex, "SUCCESS"
...
exchange, conn_status = connect_exchange(api_k, api_s, mode)

# 4. Professional UI
st.title("📊 OfficialLS AI Professional Terminal")

if conn_status == "SUCCESS":
    st.success("✅ Real Delta Connected: Trading Active")
elif mode == "Real Delta Account":
    st.error(f"❌ Connection Issue: {conn_status}")
    st.info("Tip: Delta API settings mein 'Permissions' check karein.")
...
with col_sig:
    ...
    st.success("Signal: BULLISH (72.5k Support)")
    st.info("News: BTC Market Volume Up.")
    st.divider()
